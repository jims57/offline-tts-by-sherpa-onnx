...
# License file containing the terms of use for the Kokoro model

-rw-r--r--    1 fangjun  staff    50B Feb  7 10:18 README.md
# Basic information and documentation about the model

-rw-r--r--    1 fangjun  staff    58K Feb  7 10:18 date-zh.fst
# Finite state transducer for handling date expressions in Chinese
# Converts date text (like "2023年5月1日") into normalized forms for TTS processing

drwxr-xr-x    9 fangjun  staff   288B Apr 19  2024 dict
# Dictionary directory containing additional pronunciation resources

drwxr-xr-x  122 fangjun  staff   3.8K Nov 28  2023 espeak-ng-data
# Data files from espeak-ng used for text processing and phonetic analysis
# Helps with pronunciation rules for multiple languages

-rw-r--r--    1 fangjun  staff   6.0M Feb  7 10:18 lexicon-gb-en.txt
# British English pronunciation dictionary
# Maps English words to their phonetic representations using British pronunciation

-rw-r--r--    1 fangjun  staff   5.6M Feb  7 10:18 lexicon-us-en.txt
# American English pronunciation dictionary
# Maps English words to their phonetic representations using American pronunciation

-rw-r--r--    1 fangjun  staff   2.3M Feb  7 10:18 lexicon-zh.txt
# Chinese pronunciation dictionary
# Maps Chinese characters and words to their phonetic representations

-rw-r--r--    1 fangjun  staff   310M Feb  7 10:18 model.onnx
# The main neural network model in ONNX format
# Contains the trained parameters for the Kokoro TTS system

-rw-r--r--    1 fangjun  staff    63K Feb  7 10:18 number-zh.fst
# Finite state transducer for handling number expressions in Chinese
# Converts numeric text (like "123") into spoken forms (like "一百二十三")

-rw-r--r--    1 fangjun  staff    87K Feb  7 10:18 phone-zh.fst
# Finite state transducer for handling phone number expressions in Chinese
# Normalizes phone numbers for proper pronunciation
# For example, converts "13800138000" to "一三八零零一三八零零零" for natural reading

-rw-r--r--    1 fangjun  staff   687B Feb  7 10:18 tokens.txt
# List of tokens/phonemes used by the model
# Defines the vocabulary of sounds the TTS system can produce

-rw-r--r--    1 fangjun  staff    26M Feb  7 10:18 voices.bin
# Binary file containing voice embedding vectors for all 53 speakers
# Voice embeddings are numerical representations that capture the unique characteristics of each speaker's voice
# For example, a voice embedding might be a vector like [0.2, -0.5, 0.8, ...] where each number represents 
# a specific vocal attribute (pitch, timbre, etc.). The TTS model uses these embeddings to mimic different voices.
# Enables the model to generate speech with different voice characteristics
Map between speaker ID and speaker name
This model contains 53 speakers and we use integer IDs 0-52 to represent each speaker.
...
After running, it will generate many .wav files in the current directory.
...
Please replace build/bin/sherpa-onnx-offline-tts in the above examples with python3 ./python-api-examples/offline-tts.py. or with python3 ./python-api-examples/offline-tts-play.py.
